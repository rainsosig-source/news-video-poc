...
- 한국어만 사용
- **TTS 친화적 텍스트 (절대 금지)**: 화살표(↑↓→←⇒⇨▲▼▶◀), 이모지(😀🎉🔥 등), 장식 기호(※☆★●◆■□◇○), 박스 그리기(│┃─━┌┐), 마크다운(*~`#). 수치는 자연어로 ("3.5%↑" → "3.5% 상승"). %p는 "%포인트". 발음 불가 문자는 합성 실패 원인
- **표현 태그 (선택, 권장)**: `<laugh>`, `<breath>`, `<sigh>`를 1~2줄에 자연스럽게 — 우려·아쉬움 `<sigh>`, 가벼운 반응 `<laugh>`, 주제 전환 호흡 `<breath>`. 남용 금지. 예: "지민: <sigh>이 문제는 단기로 끝날 것 같지 않네요."
- 제목/헤더/목차 없이 대화만 출력"""
